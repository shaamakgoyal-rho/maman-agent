...
| Accessibility grant **lost after every rebuild**, or Maman **asks for the keychain password every launch** | Ad-hoc signed app (unstable code identity) — macOS re-validates the TCC grant + keychain ACL each time | `bash scripts/dev-codesign.sh <Maman.app>` signs the app + sidecar with a stable self-signed "Maman Dev" identity. Grant Accessibility once and click **Always Allow** once; both then persist. Re-run after each `tauri build`. Local dev only (Developer ID is M15). |
